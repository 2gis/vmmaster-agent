...
class startScreencast(BaseScreencastCommand):
    def render_GET(self, request):
        ...
        d.addCallback(lambda r: json_response(
            {'status': r[0], 'output': "Screencast was started with PID #{}".format(self.recorder.pid)}, request)
        )
...
class stopScreencast(BaseScreencastCommand):
    def render_GET(self, request):
        ...
        d.addCallback(lambda r: json_response(
            {'status': r[0], 'output': "Screencast(PID:{}) was stopped and saved aftifact to {}".format(
                self.recorder.pid, self.recorder.screencast_dir_abspath
            )}, request)
        )
        d.addErrback(lambda f: json_response(
